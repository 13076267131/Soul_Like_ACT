...
bool UInventoryManager::RemoveEquipment(FSoulEquipmentSlot FromEquipSlot)
{
    ...
    {
        LOG_FUNC_ERROR(
            FindObject<UEnum>(ANY_PACKAGE, TEXT("UGearType"), true)->GetNameStringByIndex((int32)FromEquipSlot.
                SlotType));
        return false;
    }
}
...
bool UInventoryManager::GetInventSlots(FSoulItemData InItemData, TArray<FSoulInventSlot>& OutItemDatas,
                                       bool bSkipFullSlot /*= true*/, bool bGetEmptySlot /*= false*/) const
{
    for (auto& ItemTuple : InventoryItems)
    {
        if (bGetEmptySlot && !(ItemTuple.Value.IsValid()))
...
void UInventoryManager::EquipGear(AWeaponActor* const Inp)
{
    if (CurrentWeapon)
    {
        const FDetachmentTransformRules LocalDetRules{EDetachmentRule::KeepWorld, 1};
    ...
    const FAttachmentTransformRules LocalTransRules{EAttachmentRule::SnapToTarget, 1};
...
bool UInventoryManager::InventoryToEquipment(FSoulItemData FromItem, FSoulEquipmentSlot ToSlot)
{
    ...
    {
        const FSoulEquipmentSlot MyEquipSlot(FromItem.ItemBase->ItemSlotType);
...
void UInventoryManager::SetEquipSlot(FSoulItemData& InItemData, FSoulEquipmentSlot EquipSlot)
{
    if (InItemData.IsValid())
    {
        UE_LOG(LogTemp, Warning, TEXT("Old_ItemData-> Name:%s, Num:%i"),
               *(InItemData.ItemBase->ItemName.ToString()), InItemData.ItemCount);
    }
    else
    {
...
void UInventoryManager::SetItemSlot(UPARAM(ref) FSoulItemData& InItemData, FSoulInventSlot ItemSlot)
{
    ...
    FSoulItemData MyInventData;
    const bool bNotEmptySlot = GetInventoryItemData(ItemSlot, MyInventData);
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("New_ItemData-> Name:%s, Num:%i"),
               *(InventoryItems[ItemSlot].ItemBase->ItemName.ToString()), InventoryItems[ItemSlot].ItemCount);
    }
