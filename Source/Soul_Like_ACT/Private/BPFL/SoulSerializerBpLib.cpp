                                                ...
                                                TArray<FText>& ModifierLevels, bool& Successful)
{
    ...
    const int32 GA_Size = TargetGA.Num();
    if (GA_Size <= 0)
    {
        Successful = false;
        return;
                                                    ...
                                                    TArray<FText>& ModifierLevels, bool& Successful)
{
    ...
    USoulPrimaryStatusGameplayAbility* TempAbility = Cast<USoulPrimaryStatusGameplayAbility>(
        ItemRef->PrimaryAbility->GetDefaultObject(true));
...
void USoulSerializerBpLib::ModiferToText(
    const TPair<TSubclassOf<USoulModifierGameplayAbility>, int32>& InputAbilityInfo,
    FText& ModifierName, FText& ModifierLevel)
{
    USoulModifierGameplayAbility* TempGA = Cast<USoulModifierGameplayAbility>(
        (InputAbilityInfo.Key)->GetDefaultObject(true));
