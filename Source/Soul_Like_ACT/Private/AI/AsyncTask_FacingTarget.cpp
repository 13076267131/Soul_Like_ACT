...
UAsyncTask_FacingTarget* UAsyncTask_FacingTarget::AI_FacingTarget(UObject* WorldContextObject, float DegreeTolerance,
                                                                  float Duration, float YawSpeed, class APawn* AI_Pawn,
                                                                  class AActor* Target)
{
...
{
    float ForwardDelta;
    UBPFL_Math::FindDegreeToTarget(AI_Pawn, Target, ForwardDelta);
    ...
    {
        const FMatrix RotMatrix = FRotationMatrix(AI_Pawn->GetActorRotation());
        const FVector YawVec = RotMatrix.GetScaledAxis(EAxis::Z);
